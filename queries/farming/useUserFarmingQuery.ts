...
export async function GetFarmingData(walletAddress: string): Promise<GetFarmingData | undefined> {
	...
	try {
		...
		const combinedExchangeVolume = {
			...userVolumeData.message,
			...extractAndCombine(userVolumeData.message),
		};
		return {
			staking: userStakingData.message,
			volume: combinedExchangeVolume,
			points: userPointsData.message,
		};
	} catch (error) {
