...
export default function Profile() {
	...
	useEffect(() => {
		console.log('userFarmingQuery.data', userFarmingQuery.data);
	...
	return (
		<>
			...
			{!userTotals?.total_points || userTotals?.total_points == 0 ? (
				<div className="grow px-8 sm:px-0 flex flex-col justify-center items-center gap-6 bg-primary-light text-black">
					<h1 className="text-black text-4xl font-black text-center m-2">
						{!platformTotals?.total_points ? 'Loading...' : 'Start farming to earn points'}
					</h1>
				</div>
			) : (
				<div className="grow px-8 py-8 flex flex-col justify-center items-center bg-primary-light gap-6 text-black">
					{/* Icon */}

					{/* Your Stats heading */}
					<h1 className="animation-appear text-black text-4xl font-black text-center my-2">
						Your Positions
					</h1>

					<p className="animation-appear text-black text-2xl font-bold mt-7">Governance power</p>

					{/* Table */}
					<div
						className={classNames(
							'animation-appear hidden rounded-3xl sm:table text-center container py-3',
							styles.boxIndent
						)}
					>
						<div className="table-row">
							{['Governance power', '% of governance power', 'Users farming', 'Pool ends'].map(
								(heading, index) => (
													...
													<div>
														...
														<p>
															{formatNumberWithLocale(
																userTotals[`${room.exchange_id.toLowerCase()}`]
															)}
														</p>
													</div>
													<div>
														...
														<p>
															{formatNumberWithLocale(
																userTotals[`${room.exchange_id.toLowerCase()}_raw`]
															)}
														</p>
													</div>
													<div>
														...
														<p>
															{formatNumberWithLocale(
																platformTotals[`${room.exchange_id.toLowerCase()}_total_points`]
															)}
														</p>
													</div>
													<div>
